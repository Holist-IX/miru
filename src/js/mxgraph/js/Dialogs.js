...
var AddLinkDialog = function(editorUi, cell, btnLabel, graph)
{
	...
	mxUtils.write(inner, 'Select ports for link between switches:');

	var form = new mxForm('properties');
	form.table.style.width = '100%';
	form.table.style.padding = '10px';
	...
	let [ srcSwitch, srcSwitchPort, tgtSwitch, tgtSwitchPort ] = linkVal.split(',');

	var top = document.createElement('div');
	top.appendChild(form.table);

	var addSwitchText = function (name, value) {
		var text = document.createElement('div');
		text.style.width = '100%';
		text.style.textAlign = 'right';
		text.style.marginTop = '12px';
		mxUtils.write(text, value)
		form.addField(name, text)
	};


	var addSwitchPortsOptions = function (name, connectedPort, PortList) {
		combo = form.addCombo(name, null, null);
		combo.style.width = '100%';
		combo.style.textAlign = 'right';
		combo.style.marginTop = '6px';
		form.addOption(combo, connectedPort, connectedPort, true);
		for (port of PortList ){
			p = port.split(',')[1]
			form.addOption(combo, p, p, null)
		}
		return combo;
	}

	var srcPortsAvailable = swcon.getAvailablePorts(cell.source);
	var tgtPortsAvailable = swcon.getAvailablePorts(cell.target);

	addSwitchText("Switch 1:", srcSwitch)
	srcCombo = addSwitchPortsOptions("Switch 1 Ports:", srcSwitchPort, srcPortsAvailable)

	addSwitchText("Switch 2:", tgtSwitch)
	tgtCombo = addSwitchPortsOptions("Switch 2 Ports:", tgtSwitchPort, tgtPortsAvailable)
	...
	var mainBtn = mxUtils.button(btnLabel, function()
	{
		// Checks if there are any changes in the port selection
		newVal = `${srcSwitch},${srcCombo.value},${tgtSwitch},${tgtCombo.value}`;

		if (linkVal != newVal){
			// Update link information with new details
			cell.value.setAttribute('link', newVal);
			// Update the label
...
var ShowOutputDialog = function(editorUi, btnLabel, generate=false)
{
	...
	if (generate) {
		...
		topology = new TopologyGenerator(editorUi);
...
var ShowRollbackDialog = function(editorUi, btnLabel)
{
	var div = document.createElement('div');
	var dockerAPI = new docker(editorUi);
	...
	mxUtils.write(inner, "Rollback output");
	...
	var textNode = document.createTextNode("Rolling back the configuration that is used within cerberus.\nPlease wait....\n")
	textarea.append(textNode)
	output.appendChild(textarea);
	...
	this.init = function()
	{
		setTimeout(() => dockerAPI.rollbackCerberusConfiguration(textarea, btns), 500);
	};
	var cancelBtn = mxUtils.button(mxResources.get('cancel'), function()
	{
		editorUi.hideDialog();
