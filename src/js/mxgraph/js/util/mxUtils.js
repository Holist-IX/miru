...
var mxUtils =
{
	...
	parseYaml: function()
	{
		if (window.DOMParser)
		{
			return function(xml, graph, editorUi)
			{
				// var parser = DOMParser();
				var ymlObj = jsyaml.load(xml);

				var slices = Object.keys(ymlObj.switches).length;
				var cx = 500;
				var cy = 500;
				var height = 60;
				var width = 120;
				...
				var SB = new Sidebar(editorUi, container);

				for(var [sw, data] of Object.entries(ymlObj.switches))
				{
					var switchNode = doc.createElement("switch");
					var swi = new Object();
					swi.name = sw;
					swi.links = new Array();
					// console.log("sw: \t" + sw +"\tattr: \t" + data);
							...
							for(var [port, values] of Object.entries(val))
							{
								...
								if(values.hasOwnProperty('vlans')){
									for(var [vid, vlan] of Object.entries(values.vlans)){
										...
										portNode.setAttribute("macaddress", vlan.details.macaddresses[0]);
										portNode.setAttribute("ipv4_address", vlan.details.ipv4_addresses[0]);
										portNode.setAttribute("ipv4_address", vlan.details.ipv4_addresses[0]);
										portNode.setAttribute("ipv6_address", vlan.details.ipv6_addresses[0]);
									}
								}
					...
					var angle = i *(360/slices);
					var radian = (angle*(Math.PI/180));
					var x = (cx + rad * Math.cos(radian)) - width;
					var y = (cy + rad * Math.sin(radian)) - height;
					var style = "rounded=0;whiteSpace=wrap;html=1;";
					var id = i + 10;
					swi.id = id;
					graph.insertVertex(graph.getDefaultParent(), id, switchNode, x, y, width, height, style);
					fns.push(SB.createVertexTemplateEntry(style, width, height,switchNode, swi.name, null, null, 'rect rectangle box'))
				...
				SB.addPaletteFunctions('Switches', 'Switches', (expand != null) ? expand : true, fns);
				var arrowStyle = "edgeStyle=none;rounded=0;orthogonalLoop=1;jettySize=auto;html=1;startArrow=none;startFill=0;endArrow=none;endFill=0;"
				var linkNodes = new Object();
				for (sw of switches) {
					linkNodes[sw.name] = new Array();
					var cell = graph.getModel().getCell(sw.id);
					graph.cellLabelChanged(cell, sw.name);
					for(link of sw.links){
						if(link.link.includes(',')){
							var arr = link.link.split(',');
							node2 = arr[2];
							for (target of switches){
								if (target.name == node2){
									if(linkNodes.hasOwnProperty(node2) && !linkNodes[node2].includes(sw)){
										continue;
									}
									linkNodes[sw.name].push(target.name)
									var linkNode = doc.createElement("link");
									linkNode.setAttribute("link", link.link);
									linkNode.setAttribute("speed", link.speed);
									linkNode.setAttribute(arr[0], arr[1].split(' ')[1]);
									linkNode.setAttribute(arr[2], arr[3].split(' ')[1]);
									graph.insertEdge(graph.getDefaultParent(), null,
										linkNode, graph.getModel().getCell(sw.id),
										graph.getModel().getCell(target.id), arrowStyle);
								}
							}
						}
